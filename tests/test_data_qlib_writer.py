...
def test_write_calendar_sorts_unsorted_input(tmp_path):
    dates = [dt.date(2024, 1, 3), dt.date(2024, 1, 1), dt.date(2024, 1, 2)]
    write_calendar(tmp_path, dates)
    content = (tmp_path / "calendars" / "day.txt").read_text(encoding="utf-8")
    assert content == "2024-01-01\n2024-01-02\n2024-01-03\n"


def test_read_bin_rejects_non_integer_header(tmp_path):
    path = tmp_path / "bad.bin"
    np.array([3.7, 1.0, 2.0], dtype="<f4").tofile(path)
    with pytest.raises(ValueError, match="whole number"):
        read_bin(path)


def test_write_bin_rejects_negative_start_index(tmp_path):
    with pytest.raises(ValueError, match="start_index"):
        write_bin(tmp_path / "v.bin", [1.0], start_index=-1)
