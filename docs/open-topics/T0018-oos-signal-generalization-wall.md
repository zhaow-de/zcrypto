...
- **Basket quality is a real lever** (iter-30): a curated 10-large-cap-major gated equal-weight basket (`regime_equalweight_majors`) beats the broad 19-coin one — mean **0.493** vs 0.382, better worst (−0.444 vs −0.632); thin/newer alts drag, especially in the bear. **The best + simplest deployable strategy is now "regime-gate an equal-weight basket of the 10 large-cap majors" — no ML.** (Caution: do not grid-search the universe size to the holdout — meta-overfitting; the 10-major cut was principled.) The cheap OHLCV/regime vein is largely exhausted; new alpha needs new information (on-chain, T0010).
