...
- **`vol_target` finer-grid — CLOSED (iter-55).** Swept `vol_target` 0.40/0.45/0.55/0.60 vs `beta_null` (0.50): all deltas noise-level (vt40 +0.001, vt45 +0.003, vt55 +0.005, vt60 +0.008) — a faint monotonic drift toward higher vol_target but no CI-clearing improvement, so `vol_target=0.50` stands. **All regime-overlay research levers are now closed** (graded / vol-target / funding-stack / cross-asset-stack / anti-whipsaw / vol_target-grid); `regime_voltarget` is the ~0.31 OOS defensive ceiling. The only remaining item is the qlib upstream bug below (a tooling-tracking item, not a research lever).
