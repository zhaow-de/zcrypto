...
- [T0020 — BTC→altcoin lead-lag (intraday cross-coin predictability)](T0020-btc-alt-lead-lag.md) — the top genuine relative-alpha (Channel B) idea: whether BTC/ETH's recent intraday move predicts altcoin moves over the next hours (slow cross-coin information diffusion), tested at 1h granularity — structurally invisible to the daily-bar work that hit the T0018 wall.
...
- [T0024 — `momentum_tilt` candidate: reserved-holdout confirmation (the project's first edge)](T0024-momentum-tilt-holdout-confirmation.md) — iters 47-49 found the first overlay to beat the passive-beta null: a cross-sectional momentum tilt (+0.200), confirmed lookback-robust (14-90d) and cost-robust (2× turnover); the decisive un-peeked reserved-holdout look is human-ratified (PARKED) — this topic tees up that decision + the optional reversible pre-checks.
