...
- **Scope:** Surface a **survivorship caveat** in the experiment's outputs
  ...
  prior, and **re-scope open-topic `00005`** to reflect the data-acquisition
...
- **Addresses (partially, by framing):** open-topic `00005` (point-in-time
...
the holdout are optimistically inflated — pointing readers to `00005` for the
full analysis and fix roadmap. Nothing about the computation changes; only its
honesty does.

## Background & constraints

- **Why now / why only framing.** Investigation for `00005` established: the
...
- **Caveat scope = survivorship only.** The iter-9 methodology review flagged two
  ...
  **captured in `00002`** (its *Interpretation caveats* section); surfacing them
...
- **Repo rules unchanged.** README `## Usage` note + `docs/iterations-history.md`
  closeout + the `00005` content update; branch + PR into `develop`.
...
| Caveat set | **Survivorship only** (`00005`). CPCV-interpretation caveats now tracked in `00002` (*Interpretation caveats*); surfacing them is deferred to that topic's work (SSOT: surface only caveats that have a topic). |
...
| `00005` status | Stays **open** (no fix step done); its Findings/next-steps are sharpened to the reality. |
...
```jsonc
"caveats": [ { "topic": "00005", "summary": "universe is survivorship-biased — …" } ]
```
...
`"<recipe>: 10,000 → XXXX USDT<br><sub>⚠ survivorship-biased universe — see open-topic 00005</sub>"`.
...
`⚠ survivorship-biased universe — see open-topic 00005`. Keeping it derived from
`SURVIVORSHIP_MARKER` ensures the report subtitle and the stdout line never drift.

## SSOT & `00005` re-scope

`docs/open-topics/00005-point-in-time-universe.md` stays the canonical topic
...
- **Command tests** (redis-gated, `tests/test_experiment_command.py`): a default
  run's `run_meta.json` has a `caveats` list whose entry references topic `00005`;
...
- **Caveats unit test** (no redis, new `tests/test_experiment_caveats.py`):
  ...
  `00005` survivorship entry; `SURVIVORSHIP_MARKER` is non-empty.

## Out of scope (deferred → open-topic `00005`)
...
- Acquiring historically-delisted pairs' data; `delist`-retain / a delisted
  ...
  assumption; re-measuring PIT-vs-survivor. (All remain `00005`'s open roadmap.)
...
- `docs/open-topics/00005-point-in-time-universe.md` — the Findings/next-steps
  update above (topic stays open).

## References

- Open-topic: `docs/open-topics/00005-point-in-time-universe.md` (SSOT).
